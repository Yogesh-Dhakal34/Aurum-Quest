...
### Rarity system revived, then paused pending a bigger redesign

`GAMEPLAY.md` §4 specified a ranged rarity system (Common/Rare/Epic/Legendary with XP bands) back in Phase 1 that never actually got built — the live schema shipped with flat `Easy/Medium/Hard` difficulty and a single author-set `xp_reward` integer, no rarity field at all. Revived it as a genuinely separate, additive field (`quest_definitions.rarity`) rather than replacing difficulty, which already has its own real job (drives character stat/skill gains). Backfilled automatically from each quest's existing `xp_reward` rather than needing the live quest list reviewed by hand. Shipped: 4-tier rarity (Common/Rare/Epic/Legendary), difficulty and rarity confirmed fully decoupled (no correlation coded in anywhere).

Mid-tuning this into a deeper 6-tier version (adding Trivial/Extreme/Nightmare to difficulty, Uncommon/Mythic to rarity) when the direction changed: rather than keep incrementally extending the existing catalog-based quest system, the plan is now a more fundamental redesign — quests generated uniquely per user rather than drawn from one shared hand-authored catalog, scaffolded by a small fixed taxonomy of quest "types" still being defined. Paused here deliberately rather than build further on a foundation about to be replaced. What's actually shipped (the 4-tier rarity revival, decoupled from difficulty) stays as real progress either way — the redesign is additive scope on top of it, not a rejection of it.

### Still open for this phase

Quality pass (bug hunt, responsive/mobile QA on real devices, accessibility review, loading/error/empty-state audit beyond the pages that already handle their own), production deployment (Vercel picked as the target, not yet set up — intentionally on hold), documentation for outside readers (architecture overview, screenshots, known-limitations doc), the portfolio package (live demo, LinkedIn description, dev story, lessons-learned writeup), and the beta test itself (recruit testers, define activation/completion/return metrics) — none of these can start productively before the quality pass and deployment land first. Also now open: the quest/reward system redesign itself, pending research and a concrete vision.
